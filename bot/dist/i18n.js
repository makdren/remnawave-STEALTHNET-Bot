...
export function formatDays(n, lang = "ru") {
    if (lang !== "ru")
        return `${n} ${n === 1 ? t("day.one", lang) : t("day.many", lang)}`;
    ...
    if (last === 1)
        return `${n} ${t("day.one", lang)}`;
    if (last >= 2 && last <= 4)
        return `${n} ${t("day.few", lang)}`;
    return `${n} ${t("day.many", lang)}`;
}
