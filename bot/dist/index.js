...
import { mainMenu, backToMenu, supportSubMenu, topUpPresets, tariffPayButtons, tariffsOfCategoryButtons, tariffPaymentMethodButtons, proxyTariffPayButtons, proxyTariffsOfCategoryButtons, proxyPaymentMethodButtons, singboxTariffPayButtons, singboxTariffsOfCategoryButtons, singboxPaymentMethodButtons, topupPaymentMethodButtons, payUrlMarkup, profileButtons, extraOptionsButtons, optionPaymentMethodButtons, langButtons, currencyButtons, openSubscribePageMarkup, giftMenuButtons, giftSubscriptionButtons, giftCodeResultButtons, giftPostPurchaseButtons, giftCodesListButtons, giftTariffButtons, giftPaymentButtons, } from "./keyboard.js";
...
function subscribeKeyboard(channelInput, lang = "ru") {
    const target = parseForceChannelTarget(channelInput);
    const rows = [];
    if (target.joinUrl) {
...
async function enforceSubscription(ctx, config) {
    ...
    if (result.state === "cannot_verify") {
        await ctx.reply(`⚠️ ${msg}\n\n${_t("subscribe.cannot_verify", lang)}`, { reply_markup: subscribeKeyboard(channelId, lang) });
        return true;
    }
...
async function getOrRestoreToken(userId, username) {
    ...
    try {
        ...
        if (auth.client?.preferredLang)
            setUserLang(userId, auth.client.preferredLang);
        return auth.token;
    }
    catch {
        return null;
...
const awaitingGiftCode = new Set();
...
function buildPaymentMessage(config, vars, discount) {
    ...
    if (discount) {
        ...
        if (pos >= 0) {
            result.entities.push({ type: "strikethrough", offset: pos, length: discount.originalPrice.length }, { type: "bold", offset: pos + discount.originalPrice.length + 3, length: discount.discountedPrice.length });
        }
    }
    return result;
}
...
async function editMessageContent(ctx, text, reply_markup, entities) {
    ...
    const hasVideo = msg && typeof msg === "object" && "video" in msg && msg.video != null;
...
function getDiscountedPrice(price, discount) {
    ...
    if (discount.discountPercent && discount.discountPercent > 0)
        final -= final * discount.discountPercent / 100;
    if (discount.discountFixed && discount.discountFixed > 0)
        final -= discount.discountFixed;
...
bot.command("start", async (ctx) => {
    ...
    try {
        ...
        if (client?.preferredLang)
            setUserLang(from.id, client.preferredLang);
...
bot.on("callback_query:data", async (ctx) => {
    ...
    try {
        ...
        if (data === "check_subscribe") {
            ...
            if (channelId && config?.forceSubscribeEnabled) {
                ...
                if (result.state === "cannot_verify") {
                    ...
                    await editMessageContent(ctx, `⚠️ ${_t("subscribe.cannot_verify", lang)}`, subscribeKeyboard(channelId, lang));
                    return;
                }
                if (result.state !== "subscribed") {
        ...
        if (data === "menu:main") {
            ...
            if (client?.preferredLang)
                setUserLang(userId, client.preferredLang);
            const vpnUrl = getSubscriptionUrl(subRes.subscription);
            const showTrial = Boolean(config?.trialEnabled && !client?.trialUsed);
            const showProxy = proxyRes.items?.some((c) => c.tariffs?.length > 0) ?? false;
            const showSingbox = singboxRes.items?.some((c) => c.tariffs?.length > 0) ?? false;
        ...
        if (data === "menu:support") {
            ...
            await editMessageContent(ctx, _t("support.title", lang), supportSubMenu({
            ...
            }, config?.botBackLabel ?? null, innerStyles?.back, innerEmojiIds, lang));
            return;
        }
        ...
        if (data === "menu:devices") {
            ...
            try {
                ...
                if (devices.length === 0) {
                    await editMessageContent(ctx, _t("devices.no_devices", lang), { inline_keyboard: [[{ text: config?.botBackLabel ?? _t("back_to_menu", lang), callback_data: "menu:main" }]] });
                    return;
                }
        ...
        if (data.startsWith("devices:delete:")) {
            ...
            try {
                ...
                if (nextDevices.length === 0) {
                    await editMessageContent(ctx, _t("devices.deleted", lang), { inline_keyboard: [[{ text: config?.botBackLabel ?? _t("back_to_menu", lang), callback_data: "menu:main" }]] });
                }
                else {
        ...
        if (data === "menu:promocode") {
            ...
            await editMessageContent(ctx, _t("promo.enter_title", lang), backToMenu(config?.botBackLabel ?? null, innerStyles?.back, innerEmojiIds));
            return;
        }
        ...
        if (data === "menu:gift") {
            ...
            await editMessageContent(ctx, "🎁 Подарки и подписки\n\nЗдесь вы можете купить дополнительные подписки, подарить их или активировать подарок.", giftMenuButtons(config?.botBackLabel ?? null, innerStyles, innerEmojiIds));
            return;
        }
        if (data === "gift:buy") {
            const { items } = await api.getPublicTariffs();
            ...
            await editMessageContent(ctx, "🛒 Купить доп. подписку\n\nВыберите тариф:", giftTariffButtons(items, config?.botBackLabel ?? null, innerStyles, innerEmojiIds));
            return;
        }
        if (data.startsWith("gift_tariff:")) {
            ...
            await editMessageContent(ctx, `🛒 ${tariff.name}\n\nСтоимость: ${formatMoney(tariff.price, tariff.currency)}\n\nПодтвердите оплату:`, giftPaymentButtons(tariffId, balanceLabel, config?.botBackLabel ?? null, innerStyles, innerEmojiIds));
            return;
        }
        if (data.startsWith("gift_pay_balance:")) {
            ...
            try {
                ...
                await editMessageContent(ctx, `✅ Дополнительная подписка создана!\n\nПодписка #${result.subscriptionIndex}\n\nВы можете активировать её на своём аккаунте или подарить другу.`, giftPostPurchaseButtons(result.secondarySubscriptionId, result.subscriptionIndex, config?.botBackLabel ?? null, innerStyles, innerEmojiIds));
            }
            catch (e) {
                const msg = e instanceof Error ? e.message : "Ошибка оплаты";
                await editMessageContent(ctx, `❌ ${msg}`, backToMenu(config?.botBackLabel ?? null, innerStyles?.back, innerEmojiIds));
        ...
        if (data === "gift:subscriptions") {
            try {
                ...
                if (!result.subscriptions?.length) {
                    await editMessageContent(ctx, "📋 Мои подписки\n\nУ вас пока нет дополнительных подписок.", giftCodeResultButtons(config?.botBackLabel ?? null, innerStyles, innerEmojiIds));
                    return;
                }
                await editMessageContent(ctx, `📋 Мои подписки\n\nУ вас ${result.subscriptions.length} доп. подписок:`, giftSubscriptionButtons(result.subscriptions, config?.botBackLabel ?? null, innerStyles, innerEmojiIds));
            }
            catch (e) {
        ...
        if (data.startsWith("gift:connect:")) {
            ...
            try {
                ...
                if (config?.useRemnaSubscriptionPage) {
                    ...
                    if (!remnaUrl) {
                        await editMessageContent(ctx, "❌ Не удалось получить ссылку Remna для этой подписки.", backToMenu(config?.botBackLabel ?? null, innerStyles?.back, innerEmojiIds));
                        return;
                    }
                    await editMessageContent(ctx, `📲 Ссылка на подписку:\n\n${remnaUrl}`, openSubscribePageMarkup(appUrl2 ?? "", config?.botBackLabel ?? null, innerStyles?.back, innerEmojiIds, remnaUrl));
                    return;
                }
                ...
                await editMessageContent(ctx, `📲 Ссылка на подписку:\n\n${webUrl ?? `Подписка UUID: ${result.uuid}`}`, buttons);
            }
            catch (e) {
        ...
        if (data.startsWith("gift:give:")) {
            ...
            try {
                ...
                await editMessageContent(ctx, `🎁 Подарочный код создан!\n\nКод: \`${result.code}\`${tariffLabel}\n\nОтправьте этот код получателю или поделитесь ссылкой. Код действителен до ${expiresAt}.`, { inline_keyboard: buttons });
            }
            catch (e) {
        ...
        if (data.startsWith("gift:delete:")) {
            ...
            try {
                ...
                await editMessageContent(ctx, `✅ ${result.message || "Подписка удалена"}`, giftCodeResultButtons(config?.botBackLabel ?? null, innerStyles, innerEmojiIds));
            }
            catch (e) {
        ...
        if (data === "gift:redeem") {
            ...
            await editMessageContent(ctx, "🎁 Введите подарочный код:", backToMenu(config?.botBackLabel ?? null, innerStyles?.back, innerEmojiIds));
            return;
        }
        if (data === "gift:codes") {
            try {
                ...
                if (!result.codes?.length) {
                    await editMessageContent(ctx, "🎟️ Мои подарки\n\nУ вас пока нет подарочных кодов.", giftCodeResultButtons(config?.botBackLabel ?? null, innerStyles, innerEmojiIds));
                    return;
                }
                ...
                await editMessageContent(ctx, `🎟️ Мои подарки\n\n${lines}`, giftCodesListButtons(result.codes, config?.botBackLabel ?? null, innerStyles, innerEmojiIds));
            }
            catch (e) {
                const msg = e instanceof Error ? e.message : "Ошибка загрузки";
                await editMessageContent(ctx, `❌ ${msg}`, backToMenu(config?.botBackLabel ?? null, innerStyles?.back, innerEmojiIds));
        ...
        if (data.startsWith("gift:cancel_code:")) {
            ...
            try {
                ...
                await editMessageContent(ctx, `✅ ${result.message}`, giftCodeResultButtons(config?.botBackLabel ?? null, innerStyles, innerEmojiIds));
            }
            catch (e) {
...
bot.on("message:video", async (ctx) => {
    ...
    await ctx.reply(`📹 <b>file_id видео:</b>\n<code>${fileId}</code>\n\nСкопируйте и вставьте в админку при добавлении видео-инструкции.`, { parse_mode: "HTML" });
});
