...
const CACHE_SENTINEL = { lat: NaN, lng: NaN };
const IP_CACHE = new LRUCache({
    max: 50_000,
    ttl: 60 * 60 * 1000,
});
...
let mmLoadedPath = null;
async function getDbMaxmindPath() {
    try {
...
async function getMaxMindReader() {
    ...
    if (mmReader && mmLoadedPath === dbPath)
        return mmReader;
    if (mmLoadAttempted && mmLoadedPath === dbPath)
        return null;
    ...
    try {
        mmReader = await maxmind.open(dbPath);
...
export function resetMaxMindReader() {
    mmReader = null;
    mmLoadAttempted = false;
    mmLoadedPath = null;
}
function lookupMaxMind(ip) {
    if (!mmReader)
        return null;
    try {
...
export async function geolocateIp(ip) {
    ...
    if (cached !== undefined)
        return isSentinel(cached) ? null : cached;
