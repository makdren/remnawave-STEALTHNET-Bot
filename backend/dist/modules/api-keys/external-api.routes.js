...
import { hashPassword, verifyPassword, signClientToken, verifyClientToken, signClient2FAPendingToken, verifyClient2FAPendingToken, generateReferralCode, } from "../client/client.service.js";
import { getPublicConfig } from "../client/client.service.js";
import { isRemnaConfigured, remnaGetUser, } from "../remna/remna.client.js";
...
async function getClientOrFail(clientId, res) {
...
externalApiRouter.post("/auth/login", async (req, res) => {
...
externalApiRouter.post("/auth/2fa", async (req, res) => {
    const parsed = twoFaSchema.safeParse(req.body);
    if (!parsed.success)
        return res.status(400).json({ error: "tempToken and 6-digit code required" });
...
externalApiRouter.post("/auth/register", async (req, res) => {
...
/*  PROFILE                                    */
/* ═══════════════════════════════════════════ */
externalApiRouter.get("/client/profile", async (req, res) => {
...
externalApiRouter.patch("/client/profile", async (req, res) => {
    const clientId = requireClientToken(req, res);
    if (!clientId)
        return;
...
/*  BALANCE & PAYMENTS                         */
/* ═══════════════════════════════════════════ */
externalApiRouter.get("/client/balance", async (req, res) => {
    const clientId = requireClientToken(req, res);
    if (!clientId)
        return;
    const client = await prisma.client.findUnique({
...
externalApiRouter.get("/client/payments", async (req, res) => {
    const clientId = requireClientToken(req, res);
    if (!clientId)
        return;
...
/*  SUBSCRIPTION                               */
/* ═══════════════════════════════════════════ */
externalApiRouter.get("/client/subscription", async (req, res) => {
    const clientId = requireClientToken(req, res);
    if (!clientId)
        return;
    const client = await prisma.client.findUnique({
...
/*  REFERRALS                                  */
/* ═══════════════════════════════════════════ */
externalApiRouter.get("/client/referrals", async (req, res) => {
    const clientId = requireClientToken(req, res);
    if (!clientId)
        return;
    const client = await prisma.client.findUnique({
...
externalApiRouter.get("/tariffs", async (_req, res) => {
...
externalApiRouter.get("/proxy-tariffs", async (_req, res) => {
...
externalApiRouter.get("/singbox-tariffs", async (_req, res) => {
...
/*  CONFIG (public)                            */
/* ═══════════════════════════════════════════ */
externalApiRouter.get("/config", async (_req, res) => {
...
/*  DEVICES (HWID)                             */
/* ═══════════════════════════════════════════ */
externalApiRouter.get("/client/devices", async (req, res) => {
    const clientId = requireClientToken(req, res);
    if (!clientId)
        return;
    const client = await prisma.client.findUnique({
...
/*  PROXY & SINGBOX SLOTS                      */
/* ═══════════════════════════════════════════ */
externalApiRouter.get("/client/proxy-slots", async (req, res) => {
    const clientId = requireClientToken(req, res);
    if (!clientId)
        return;
...
externalApiRouter.get("/client/singbox-slots", async (req, res) => {
    const clientId = requireClientToken(req, res);
    if (!clientId)
        return;
