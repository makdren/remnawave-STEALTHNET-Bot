...
function startWithExpression(cronExpression) {
    ...
    const task = cron.schedule(schedule, async () => {
        ...
        try {
            ...
            if (results.length === 0) {
                ...
                console.log(`[auto-broadcast] Cron run complete: ${results.length} rule(s), ${total} sent, ${totalErrors} error(s)`);
...
export async function startAutoBroadcastScheduler(cronExpression) {
...
export async function restartAutoBroadcastScheduler() {
    if (currentTask) {
        currentTask.stop();
        currentTask = null;
    }
...
export function stopAutoBroadcastScheduler() {
    if (currentTask) {
        currentTask.stop();
        currentTask = null;
    }
}
//# sourceMappingURL=auto-broadcast-scheduler.js.map