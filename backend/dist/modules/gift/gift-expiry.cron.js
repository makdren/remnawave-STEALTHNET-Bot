 ...
 */
async function notifyExpiringGiftCodes() {
    ...
    for (const code of codes) {
        ...
        await sendTelegramNotification(telegramId, `⏰ Ваш подарочный код <b>${code.code}</b> (${tariffLabel}) истечёт через ${timeLabel}.\n\nЕсли получатель не активирует код, подписка вернётся вам.`);
        await prisma.giftCode.update({
            where: { id: code.id },
            data: { expiryNotifiedAt: new Date() },
        });
...
//# sourceMappingURL=gift-expiry.cron.js.map