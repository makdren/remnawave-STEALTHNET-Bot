...
import { remnaCreateUser, remnaUsernameFromClient, extractRemnaUuid, isRemnaConfigured, remnaDeleteUser, } from "../remna/remna.client.js";
...
async function getNextSubscriptionIndex(ownerId) {
...
function secondaryRemnaUsername(rootClient, index) {
...
async function logGiftEvent(clientId, eventType, secondarySubscriptionId, metadata) {
 ...
 */
export async function createAdditionalSubscription(rootClientId, tariff, options) {
    ...
    const trafficLimitStrategy = trafficResetMode === "monthly" ? "MONTH" : trafficResetMode === "monthly_rolling" ? "MONTH_ROLLING" : "NO_RESET";
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
        ...
        if (remnaUuid)
            break;
        const isUsernameTaken = createRes.status === 400 &&
 ...
 */
export async function activateForSelf(ownerId, subscriptionId) {
 ...
 */
export async function deleteSubscription(ownerId, subscriptionId) {
    const sub = await prisma.secondarySubscription.findUnique({
        where: { id: subscriptionId },
        include: { tariff: { select: { name: true } } },
    });
 ...
 */
export async function listClientSubscriptions(rootClientId) {
 ...
 */
export async function listAllClientSubscriptions(rootClientId) {
    const secondaries = await prisma.secondarySubscription.findMany({
        where: {
            ownerId: rootClientId,
            OR: [
 ...
 */
export async function createGiftCode(rootClientId, secondarySubscriptionId, giftMessage, options) {
    const config = await getSystemConfig();
    if (!options?.skipConfigCheck && !config.giftSubscriptionsEnabled) {
 ...
 */
export async function redeemGiftCode(recipientRootClientId, rawCode) {
    const config = await getSystemConfig();
    ...
    if (creator?.telegramId) {
        ...
        sendTelegramNotification(creator.telegramId, `🎁 Ваш подарок активирован!\n\n${recipientName} принял(а) ваш подарок${tariffLabel}.`);
    }
    return {
        ok: true,
        data: {
 ...
 */
export async function cancelGiftCode(rootClientId, codeOrId) {
 ...
 */
async function expireGiftCode(giftCodeId, secondarySubscriptionId) {
    await prisma.$transaction([
        prisma.giftCode.update({
            where: { id: giftCodeId },
            data: { status: "EXPIRED" },
 ...
 */
export async function expireOldGiftCodes() {
 ...
 */
export async function listGiftCodes(rootClientId) {
 ...
 */
export async function getSubscriptionUrl(subscriptionId, rootClientId) {
    const sub = await prisma.secondarySubscription.findUnique({
        where: { id: subscriptionId },
 ...
 */
export async function getGiftHistory(clientId, page = 1, limit = 20) {
 ...
 */
export async function getPublicGiftCodeInfo(rawCode) {
    const normalized = normalizeCode(rawCode);
 ...
 */
export async function adminCreateGiftCode(ownerClientId, tariffId, giftMessage) {
    // Находим тариф
    ...
    const codeResult = await createGiftCode(ownerClientId, subResult.data.secondarySubscriptionId, giftMessage, { skipConfigCheck: true });
    if (!codeResult.ok) {
        return codeResult;
    }
