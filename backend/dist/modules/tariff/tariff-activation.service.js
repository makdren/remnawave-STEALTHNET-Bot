...
import { remnaCreateUser, remnaUpdateUser, remnaGetUser, isRemnaConfigured, remnaGetUserByTelegramId, remnaGetUserByEmail, extractRemnaUuid, remnaUsernameFromClient, remnaResetUserTraffic, } from "../remna/remna.client.js";
...
function extractCurrentSquads(data) {
    if (!data || typeof data !== "object")
        return [];
 ...
 */
async function mergeSquads(tariffSquadUuids, currentSquadUuids) {
...
function remnaStrategy(mode) {
    if (mode === "monthly")
        return "MONTH";
    if (mode === "monthly_rolling")
        return "MONTH_ROLLING";
    return "NO_RESET";
}
/**
...
function computeConvertedDays(args) {
    ...
    if (oldPricePerDay == null || oldPricePerDay <= 0)
        return 0;
    if (newPricePerDay <= 0)
        return remainingDays; // free → всё бесплатно, отдаём как есть
 ...
 */
export async function activateTariffForClient(client, tariff, selectedOption) {
    ...
    const resetMode = tariff.trafficResetMode || "no_reset";
...
function isAdditionalSubscriptionPayment(metadata) {
    if (!metadata?.trim())
        return false;
    try {
...
function parseCustomBuildMetadata(metadata) {
    if (!metadata?.trim())
        return null;
    try {
        const o = JSON.parse(metadata);
        const cb = o?.customBuild;
        ...
        const internalSquadUuids = Array.isArray(cb.internalSquadUuids)
            ? cb.internalSquadUuids.filter((u) => typeof u === "string" && u.trim())
            : [];
        const trafficLimitBytes = typeof cb.trafficLimitBytes === "number" && cb.trafficLimitBytes >= 0
            ? BigInt(Math.floor(cb.trafficLimitBytes))
            : typeof cb.trafficLimitBytes === "string"
                ? BigInt(cb.trafficLimitBytes)
                : null;
        if (durationDays < 1 || internalSquadUuids.length === 0)
            return null;
