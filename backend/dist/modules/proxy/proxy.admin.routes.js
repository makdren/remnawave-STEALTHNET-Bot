...
proxyAdminRouter.patch("/tariffs/:id", asyncRoute(async (req, res) => {
    ...
    if (body.data.categoryId !== undefined)
        data.categoryId = body.data.categoryId;
    if (body.data.proxyCount !== undefined)
        data.proxyCount = body.data.proxyCount;
    if (body.data.durationDays !== undefined)
    ...
    if (body.data.connectionLimit !== undefined)
        data.connectionLimit = body.data.connectionLimit;
    if (body.data.price !== undefined)
        data.price = body.data.price;
    if (body.data.currency !== undefined)
        data.currency = body.data.currency.toUpperCase();
    ...
    const updated = await prisma.$transaction(async (tx) => {
        const tariff = await tx.proxyTariff.update({ where: { id }, data: data });
        if (body.data.nodeIds !== undefined) {
            await tx.proxyTariffNode.deleteMany({ where: { tariffId: id } });
            const nodeIds = body.data.nodeIds;
            if (nodeIds && nodeIds.length > 0) {
                await tx.proxyTariffNode.createMany({
                    data: nodeIds.map((nodeId) => ({ tariffId: id, nodeId })),
                    skipDuplicates: true,
                });
            }
        }
...
proxyAdminRouter.patch("/slots/:id", asyncRoute(async (req, res) => {
    ...
    const data = {};
    if (body.data.login !== undefined)
        data.login = body.data.login.trim();
    ...
    const updated = await prisma.proxySlot.update({ where: { id }, data: data });
    return res.json({
        id: updated.id,
        login: updated.login,
...
proxyAdminRouter.get("/slots/export", asyncRoute(async (req, res) => {
    ...
    const rows = slots.map((s) => {
        const escape = (v) => v == null ? "" : String(v).replace(/;/g, ",").replace(/\n/g, " ");
        return [
            s.node.id,
            escape(s.node.name),
...
//# sourceMappingURL=proxy.admin.routes.js.map