...
export async function markPaymentPaid(paymentId) {
    ...
    const isTopUp = (payment.provider === "yoomoney_form" || payment.provider === "platega" || payment.provider === "yookassa") &&
        !payment.tariffId &&
    ...
    let proxySlots = { ok: false };
    if (isExtraOption) {
        const extraResult = await applyExtraOptionByPaymentId(paymentId);
        activation = extraResult.ok ? { ok: true } : { ok: false, error: extraResult.error };
    }
...
//# sourceMappingURL=mark-paid.service.js.map