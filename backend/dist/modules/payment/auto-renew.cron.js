...
import { notifyAutoRenewSuccess, notifyAutoRenewFailed, notifyAutoRenewUpcoming, notifyAutoRenewRetry, notifyAutoRenewYookassaSuccess, notifyAutoRenewYookassaFailed, } from "../notification/telegram-notify.service.js";
/**
 ...
 */
async function tryApplyPromoForAutoRenew(clientId, code, basePrice) {
    if (!code?.trim())
...
export async function processAutoRenewals() {
    ...
    for (const client of clients) {
        if (!client.remnawaveUuid || !client.autoRenewTariff)
            continue;
        try {
            ...
            const userData = remnaUser.data?.response ?? remnaUser.data;
            if (!userData || typeof userData !== "object")
                continue;
            const expireAtRaw = userData.expireAt;
            if (!expireAtRaw)
                continue;
            const expireAtDate = new Date(expireAtRaw);
            ...
            if (timeLeft > 0 && timeLeft <= notifyThreshold) {
                const shouldNotify = !client.autoRenewNotifiedAt ||
                    now - client.autoRenewNotifiedAt.getTime() > DAY_MS;
                if (shouldNotify && client.balance < client.autoRenewTariff.price) {
                    await notifyAutoRenewUpcoming(client.id, client.autoRenewTariff.name, client.autoRenewTariff.price, client.autoRenewTariff.currency, Math.max(0, Math.ceil(timeLeft / DAY_MS)));
            ...
            if (timeLeft <= renewThreshold && timeLeft >= -(3 * DAY_MS)) {
                ...
                const { finalPrice: tariffPrice, promoCodeId: autoRenewPromoCodeId } = await tryApplyPromoForAutoRenew(client.id, client.autoRenewPromoCode, baseTariffPrice);
                if (client.balance >= tariffPrice) {
                    ...
                    await notifyAutoRenewSuccess(client.id, client.autoRenewTariff.name, tariffPrice, client.autoRenewTariff.currency);
                    ...
                    if (config.yookassaRecurringEnabled &&
                        ...
                        if (recentAutopay) {
                            console.log(`[auto-renew] Client ${client.id}: found recent PAID YooKassa autopay ${recentAutopay.id}, retrying tariff activation only (no new charge).`);
                            ...
                            if (activationRes.ok) {
                                ...
                                await notifyAutoRenewYookassaSuccess(client.id, client.autoRenewTariff.name, recentAutopay.amount, client.autoRenewTariff.currency, client.yookassaPaymentMethodTitle ?? undefined, undefined, recentAutopay.amount);
                                console.log(`[auto-renew] Client ${client.id} tariff activated from recent autopay ${recentAutopay.id}.`);
                            }
                            else {
                                console.error(`[auto-renew] Client ${client.id}: recent autopay ${recentAutopay.id} STILL failing activation: ${activationRes.error}`);
                            }
                            ...
                            if (autopayResult.ok) {
                                ...
                                for (let attempt = 1; attempt <= 2 && !activationRes.ok; attempt++) {
                                    console.warn(`[auto-renew] Client ${client.id}: tariff activation attempt ${attempt} failed for ${payment.id}: ${activationRes.error}. Retrying...`);
                                ...
                                if (activationRes.ok) {
                                    ...
                                    import("../referral/referral.service.js")
                                        .then((m) => m.distributeReferralRewards(payment.id))
                                        .catch((e) => console.error("[auto-renew] Referral reward error:", e));
                                    await notifyAutoRenewYookassaSuccess(client.id, client.autoRenewTariff.name, tariffPrice, client.autoRenewTariff.currency, client.yookassaPaymentMethodTitle ?? undefined, balancePortion > 0 ? balancePortion : undefined, cardPortion);
                                    ...
                                    console.error(`[auto-renew] Client ${client.id}: YooKassa PAID (${payment.id}) but tariff activation failed after retries: ${activationRes.error}. Will retry activation on next cron run without re-charging.`);
                                }
                                ...
                                await notifyAutoRenewYookassaFailed(client.id, client.autoRenewTariff.name, autopayResult.error);
                    ...
                    if (!yookassaPaid) {
                        ...
                        if (currentRetryCount < maxRetries) {
                            ...
                            await notifyAutoRenewRetry(client.id, client.autoRenewTariff.name, tariffPrice, client.autoRenewTariff.currency, newRetryCount, maxRetries);
                            console.log(`[auto-renew] Client ${client.id} insufficient balance. Retry ${newRetryCount}/${maxRetries}.`);
                        }
                        else {
                            ...
                            if (expiredSince >= gracePeriod) {
                                ...
                                console.log(`[auto-renew] Client ${client.id} failed: all retries exhausted + grace period over. Auto-renew disabled.`);
                            }
                            else {
                                // Still within grace period — keep trying each hour
                                console.log(`[auto-renew] Client ${client.id} retries exhausted but grace period active. Will keep checking.`);
                            }
                        }
                    }
                }
...
//# sourceMappingURL=auto-renew.cron.js.map