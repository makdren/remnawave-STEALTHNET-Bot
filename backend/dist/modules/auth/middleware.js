...
function normaliseAdminPath(req) {
    let path = (req.path || req.originalUrl || "").replace(/^\//, "");
    if (path.startsWith("api/admin/"))
        path = path.slice("api/admin/".length);
    else if (path === "api/admin" || path.startsWith("api/admin"))
        path = path.slice("api/admin".length).replace(/^\//, "");
    return path;
}
/** Путь API admin -> раздел доступа (первый сегмент пути с маппингом). */
function getSectionFromPath(normalisedPath) {
    ...
    if (first === "payments")
        return "sales-report";
    if (first === "tariff-categories")
        return "tariffs";
    if (first === "default-subscription-page-config")
        return "settings";
    if (first === "sync")
        return "settings";
    if (first === "promo-groups")
        return "promo";
    if (first === "referrals")
        return "clients";
    if (first === "traffic-abuse")
        return "analytics";
    if (first === "api-keys")
        return "settings";
    if (first === "gramads")
...
export async function requireAuth(req, res, next) {
    ...
    try {
        ...
        ext.adminRole = (admin.role === "MANAGER" ? "MANAGER" : "ADMIN");
...
export function requireAdminSection(req, res, next) {
    const ext = req;
    ...
    if (ext.adminAllowedSections.includes(section))
        return next();
