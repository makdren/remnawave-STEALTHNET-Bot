...
import { verifyPassword, signAccessToken, signRefreshToken, verifyToken, hashPassword, signAdmin2FAPendingToken, verifyAdmin2FAPendingToken, } from "./auth.service.js";
...
authRouter.post("/2fa/confirm", requireAuth, async (req, res) => {
    ...
    if (!row?.totpSecret || row.totpEnabled)
        return res.status(400).json({ message: "Сначала запустите настройку 2FA или 2FA уже включена" });
