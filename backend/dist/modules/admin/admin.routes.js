...
import { notifyAdminsAboutSupportReply, notifyAdminsAboutTicketStatusChange, } from "../notification/telegram-notify.service.js";
...
adminRouter.get("/me", asyncRoute(async (req, res) => {
    ...
    const allowedSections = admin.allowedSections
        ? (() => {
            try {
                const p = JSON.parse(admin.allowedSections);
...
const PAID_EXTERNAL_WHERE = { status: "PAID", provider: { not: "balance" } };
...
adminRouter.get("/dashboard/stats", async (_req, res) => {
    ...
    const [clientsTotal, clientsWithRemna, paidAgg, paidToday, paidLast7, paidLast30, newClientsToday, newClientsLast7, newClientsLast30] = await Promise.all([
        prisma.client.count(),
...
adminRouter.get("/server/ssh", asyncRoute(async (_req, res) => {
    ...
    if (!config)
        return res.status(404).json({ message: "sshd_config не найден (SSH не настроен или нет доступа)" });
...
adminRouter.patch("/server/ssh", asyncRoute(async (req, res) => {
    ...
    const updates = {};
    if (body.port != null)
        updates.port = Number(body.port);
    if (body.permitRootLogin != null)
        updates.permitRootLogin = String(body.permitRootLogin);
    if (body.passwordAuthentication != null)
        updates.passwordAuthentication = Boolean(body.passwordAuthentication);
    if (body.pubkeyAuthentication != null)
        updates.pubkeyAuthentication = Boolean(body.pubkeyAuthentication);
    const result = await updateSshConfig(updates);
    if (!result.ok)
...
adminRouter.get("/auto-renew/stats", async (_req, res) => {
    ...
    const [totalAutoRenewEnabled, totalAutoRenewDisabled, retriesInProgress, renewalsLast30Days, renewalsLast7Days, renewalAmountLast30Days,] = await Promise.all([
...
const TRAFFIC_RESET_MODES = ["no_reset", "on_purchase", "monthly", "monthly_rolling"];
...
adminRouter.patch("/tariffs/:id", async (req, res) => {
    ...
    if (body.data.trafficResetMode !== undefined)
        data.trafficResetMode = body.data.trafficResetMode;
...
adminRouter.get("/clients", async (req, res) => {
    try {
        ...
        if (search.length > 0) {
            conditions.push({
                OR: [
                    ...
                    { id: { contains: search } },
                    { remnawaveUuid: { contains: search, mode: "insensitive" } },
                ],
            });
        }
        ...
        if (isRemnaConfigured()) {
            ...
            const map = {};
            await Promise.all(withRemna.map(async (c) => {
                try {
...
adminRouter.post("/clients/online-statuses", async (req, res) => {
    try {
        ...
        const result = {};
        if (!isRemnaConfigured()) {
            for (const uuid of limited)
                result[uuid] = { onlineAt: null };
            return res.json(result);
        }
        await Promise.all(limited.map(async (uuid) => {
            try {
...
function getRemnaUserFieldsForMerge(data) {
    if (!data || typeof data !== "object")
        return { activeInternalSquads: [] };
    const o = data;
...
adminRouter.patch("/clients/:id/remna", async (req, res) => {
    ...
    if (getRes.error)
        return res.status(getRes.status >= 400 ? getRes.status : 500).json({ message: getRes.error });
    const current = getRemnaUserFieldsForMerge(getRes.data);
    const patchBody = { uuid: remnaUuid };
    if (body.data.activeInternalSquads === undefined && current.activeInternalSquads.length > 0)
        patchBody.activeInternalSquads = current.activeInternalSquads;
    if (body.data.telegramId === undefined && current.telegramId !== undefined)
        patchBody.telegramId = current.telegramId;
    if (body.data.email === undefined && current.email !== undefined)
        patchBody.email = current.email;
...
adminRouter.post("/clients/:id/remna/unlink", async (req, res) => {
    ...
    if (!client.remnawaveUuid)
        return res.status(400).json({ message: "Клиент уже не привязан к Remna" });
...
adminRouter.post("/broadcast", broadcastUpload.single("attachment"), asyncRoute(async (req, res) => {
    const parsed = broadcastSchema.safeParse(req.body);
    if (!parsed.success) {
        return res.status(400).json({ message: "Invalid input", errors: parsed.error.flatten() });
    }
    const { channel, subject, message, buttonText, buttonUrl } = parsed.data;
    const attachment = req.file && req.file.buffer
...
adminRouter.get("/broadcast/status/:jobId", asyncRoute(async (req, res) => {
...
adminRouter.get("/analytics", async (_req, res) => {
    ...
    const isExternal = (provider) => provider !== "balance";
    ...
    const bySourceKey = {};
    function keyFor(source, campaign) {
    ...
    for (const c of allClients) {
        ...
        if (!bySourceKey[k])
            bySourceKey[k] = { registrations: 0, trials: 0, payments: 0, revenue: 0 };
        bySourceKey[k].registrations++;
        if (c.trialUsed)
            bySourceKey[k].trials++;
    }
...
adminRouter.get("/video-instructions", async (_req, res) => {
    ...
    let items = [];
    try {
        items = JSON.parse(dataRow?.value || "[]");
    }
    catch { /* empty */ }
...
adminRouter.post("/video-instructions", async (req, res) => {
    ...
    if (!title || !telegramFileId)
        return res.status(400).json({ error: "title and telegramFileId required" });
...
adminRouter.put("/video-instructions/:id", async (req, res) => {
    ...
    if (telegramFileId !== undefined)
        items[idx].telegramFileId = String(telegramFileId).trim();
    await prisma.systemSetting.upsert({
        where: { key: "video_instructions" },
        create: { key: "video_instructions", value: JSON.stringify(items) },
        update: { value: JSON.stringify(items) },
...
adminRouter.post("/admins", asyncRoute(async (req, res) => {
    ...
    const sections = (body.data.allowedSections ?? []).filter((s) => ADMIN_ALLOWED_SECTIONS.includes(s));
...
adminRouter.patch("/admins/:id", asyncRoute(async (req, res) => {
    ...
    const allowed = updated.allowedSections
        ? (() => {
            try {
                const p = JSON.parse(updated.allowedSections);
                return Array.isArray(p) ? p.filter((s) => typeof s === "string") : [];
            }
            catch {
                return [];
    ...
    if (search.length > 0) {
        conditions.push({
            OR: [
                ...
                { giftedToClient: { email: { contains: search, mode: "insensitive" } } },
                { giftedToClient: { telegramUsername: { contains: search, mode: "insensitive" } } },
...
adminRouter.delete("/secondary-subscriptions/bulk", asyncRoute(async (req, res) => {
    ...
    if (isRemnaConfigured()) {
        await Promise.allSettled(subs.filter((s) => s.remnawaveUuid).map((s) => remnaDeleteUser(s.remnawaveUuid)));
    }
    // Log events
...
adminRouter.get("/gift-analytics", asyncRoute(async (_req, res) => {
    const now = new Date();
    const thirtyDaysAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
    const [totalSubscriptions, last30Days, activatedSelf, gifted, pendingCodes, expiredCodes, redeemedCodes, totalCodes,] = await Promise.all([
...
const TOUR_PLACEMENTS = ["top", "bottom", "left", "right", "center"];
const TOUR_MOODS = ["wave", "point", "happy", "think"];
...
adminRouter.patch("/tour-steps/reorder", asyncRoute(async (req, res) => {
    ...
    if (!body.success)
        return res.status(400).json({ message: "Неверные данные", errors: body.error.flatten() });
    await prisma.$transaction(body.data.items.map(item => prisma.tourStep.update({ where: { id: item.id }, data: { sortOrder: item.sortOrder } })));
    return res.json({ success: true });
}));
adminRouter.patch("/tour-steps/:id", asyncRoute(async (req, res) => {
    ...
    if (body.data.target !== undefined)
        data.target = body.data.target;
    if (body.data.targetLabel !== undefined)
        data.targetLabel = body.data.targetLabel;
    ...
    if (body.data.placement !== undefined)
        data.placement = body.data.placement;
    if (body.data.route !== undefined)
        data.route = body.data.route;
    if (body.data.mascotId !== undefined)
        data.mascotId = body.data.mascotId;
    if (body.data.mood !== undefined)
        data.mood = body.data.mood;
    if (body.data.sortOrder !== undefined)
        data.sortOrder = body.data.sortOrder;
...
adminRouter.post("/tour-steps/seed-defaults", asyncRoute(async (_req, res) => {
    ...
    const created = await prisma.$transaction(defaults.map(step => prisma.tourStep.create({ data: step })));
...
adminRouter.patch("/tour-mascots/:id", asyncRoute(async (req, res) => {
    ...
    if (!mascot)
        return res.status(404).json({ message: "Маскот не найден" });
...
adminRouter.delete("/tour-mascots/:id", asyncRoute(async (req, res) => {
    ...
    if (!mascot)
        return res.status(404).json({ message: "Маскот не найден" });
    if (mascot.isBuiltIn)
        return res.status(400).json({ message: "Нельзя удалить встроенного маскота" });
    ...
    if (filename)
        removeUploadedFile(`mascots/${filename}`);
    return res.json({ success: true });
}));
// ——————————————————————————————————————————————————————————
// Tour Mascot Emotions CRUD
// ——————————————————————————————————————————————————————————
const validMoods = ["wave", "point", "happy", "think"];
adminRouter.post("/tour-mascots/:id/emotions", uploadMascotImage.single("image"), asyncRoute(async (req, res) => {
    ...
    if (existing) {
        ...
        if (oldFilename)
            removeUploadedFile(`mascots/${oldFilename}`);
...
adminRouter.delete("/tour-mascots/:id/emotions/:emotionId", asyncRoute(async (req, res) => {
    ...
    if (!emotion)
        return res.status(404).json({ message: "Эмоция не найдена" });
...
adminRouter.post("/tour-steps/:id/video", uploadVideo.single("video"), asyncRoute(async (req, res) => {
    ...
    if (step.videoUrl?.startsWith("/api/uploads/videos/")) {
        ...
        if (oldFilename)
            removeUploadedFile(`videos/${oldFilename}`);
    }
    const updated = await prisma.tourStep.update({
        where: { id },
