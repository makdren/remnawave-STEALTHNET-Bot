...
import { remnaGetUsers, remnaGetNodes, remnaGetNodeUsersUsage, isRemnaConfigured, } from "../remna/remna.client.js";
 ...
 */
trafficAbuseRouter.get("/analytics", async (req, res) => {
    if (!isRemnaConfigured()) {
        return res.status(503).json({ message: "Remna API not configured" });
    }
    const days = Math.min(Math.max(parseInt(req.query.days) || 7, 1), 90);
    const threshold = parseFloat(req.query.threshold) || 0.7;
    const minBytes = parseInt(req.query.minBytes) || 100_000_000;
    try {
        ...
        const nodeUsageResults = await Promise.allSettled(activeNodes.map((node) => remnaGetNodeUsersUsage(node.uuid, startStr, endStr, 500).catch((e) => {
        ...
        const usernameToNodeUsage = new Map();
        let nodesWithData = 0;
        activeNodes.forEach((node, i) => {
            ...
            for (const u of topUsers) {
                if (!u.username || typeof u.total !== "number")
                    continue;
        ...
        for (const [username, nodeUsages] of usernameToNodeUsage) {
            ...
            if (limit > 0 && usagePercent < threshold * 100)
                continue;
            if (limit === 0 && abuseScore < 150)
                continue;
            abusers.push({
                uuid: user?.uuid ?? "",
                username,
        ...
        const totalTrafficPeriod = Array.from(usernameToNodeUsage.values()).reduce((sum, entries) => sum + entries.reduce((s, e) => s + e.bytes, 0), 0);
        ...
        res.json({
            ...
            stats: {
                ...
                abuserTrafficTotal,
                abuserTrafficPercent: totalTrafficPeriod > 0
...
function computeGlobalAvg(map, days) {
    let totalBytes = 0;
    let count = 0;
...
//# sourceMappingURL=traffic-abuse.routes.js.map