...
adminGramadsRouter.get("/status", async (_req, res) => {
    ...
    try {
        ...
        if (!r.ok)
            return res.json({ configured: true, valid: false, error: `HTTP ${r.status}` });
...
adminGramadsRouter.all(/^\/proxy\/(.+)$/, async (req, res) => {
    const key = await getApiKey();
    if (!key)
        return res.status(400).json({ error: "gramads_api_key is not configured" });
    ...
    try {
        ...
        if (!r.ok) {
            ...
            console.warn(`[gramads] ${req.method} ${normalized} -> HTTP ${r.status} | req=${safeBody} | resp=${text.slice(0, 500)}`);
        }
        else if (ct.includes("application/json")) {
            try {
                const parsed = JSON.parse(text);
                const expl = parsed?.notSuccessExplanation;
                if (typeof expl === "number" && expl !== 0) {
                    const safeBody = body ? body.slice(0, 500) : "";
                    console.warn(`[gramads] ${req.method} ${normalized} -> notSuccessExplanation=${expl} | req=${safeBody}`);
                }
            }
            catch {
...
//# sourceMappingURL=gramads.routes.js.map