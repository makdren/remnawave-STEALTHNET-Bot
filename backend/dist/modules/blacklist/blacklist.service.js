...
const BLACKLIST_URL = "https://raw.githubusercontent.com/BEDOLAGA-DEV/VPN-BLACKLIST/main/blacklist.txt";
const CACHE_TTL_MS = 30 * 60 * 1000;
let cachedSet = null;
let lastFetchMs = 0;
function parseBlacklistText(raw) {
    const set = new Set();
 ...
 */
export async function checkAndBlockIfBlacklisted(telegramId) {
    const set = await getBlacklistSet();
    if (!set.has(telegramId))
        return false;
