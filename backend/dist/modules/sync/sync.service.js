...
import { isRemnaConfigured, remnaGetUsers, remnaGetUser, remnaUpdateUser, remnaCreateUser, remnaGetUserByTelegramId, remnaGetUserByEmail, remnaGetUserByUsername, extractRemnaUuid, remnaUsernameFromClient, } from "../remna/remna.client.js";
            ...
            for (const u of users) {
                ...
                try {
                    ...
                    if (existing) {
                        ...
                        if (username && !existing.telegramUsername)
...
function extractRemnaUserFields(data) {
    if (!data || typeof data !== "object")
    ...
    if (Array.isArray(ais)) {
        for (const x of ais) {
            if (typeof x === "string" && x.trim())
                squads.push(x.trim());
            else if (x && typeof x === "object" && typeof x.uuid === "string")
                squads.push(x.uuid);
        }
    }
    return {
...
function isRemnaNotFoundError(status, error) {
...
async function remnaGetUserWithRetry(uuid) {
    const first = await remnaGetUser(uuid);
    if (!first.error || first.status !== 500)
        return first;
...
export async function syncToRemna() {
    const result = { updated: 0, unlinked: 0, errors: [] };
    if (!isRemnaConfigured()) {
