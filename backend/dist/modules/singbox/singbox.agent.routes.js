...
singboxAgentRouter.post("/register", asyncRoute(async (req, res) => {
    ...
    const publicHost = req.headers["x-forwarded-for"]?.split(",")[0]?.trim()
...
singboxAgentRouter.post("/:id/heartbeat", requireNodeToken, asyncRoute(async (req, res) => {
    ...
    if (body.data.connections !== undefined)
        updateData.currentConnections = body.data.connections;
    if (body.data.trafficIn !== undefined)
        updateData.trafficInBytes = BigInt(body.data.trafficIn);
    if (body.data.trafficOut !== undefined)
        updateData.trafficOutBytes = BigInt(body.data.trafficOut);
    await prisma.singboxNode.update({
        where: { id: nodeId },
        data: updateData,
    });
    if (body.data.slots?.length) {
        for (const s of body.data.slots) {
            const slotUpdate = {};
            if (s.trafficUsed !== undefined)
                slotUpdate.trafficUsedBytes = BigInt(s.trafficUsed);
            if (s.connections !== undefined)
                slotUpdate.currentConnections = s.connections;
...
//# sourceMappingURL=singbox.agent.routes.js.map