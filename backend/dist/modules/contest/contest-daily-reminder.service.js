...
async function sendTelegram(botToken, chatId, text, replyMarkup) {
    ...
    try {
        ...
        if (SKIP_PATTERNS.some((p) => p.test(desc)))
            return { ok: false, skip: true, error: desc };
...
function formatPrizeLine(prizeType, prizeValue) {
    const v = (prizeValue || "").trim();
    if (!v)
        return "—";
    if (prizeType === "balance")
        return `${v} ₽ на баланс`;
    if (prizeType === "vpn_days")
        return `${v} дней VPN`;
    return v;
}
function buildContestStartMessage(contest) {
    ...
    lines.push("", "<b>🎁 Призы:</b>", `1 место — ${formatPrizeLine(contest.prize1Type, contest.prize1Value)}`, `2 место — ${formatPrizeLine(contest.prize2Type, contest.prize2Value)}`, `3 место — ${formatPrizeLine(contest.prize3Type, contest.prize3Value)}`);
    return lines.join("\n");
}
async function broadcastToClients(botToken, clients, text, replyMarkup) {
    let sent = 0;
    let errors = 0;
    let skipped = 0;
...
export async function runContestDailyReminder() {
    const now = new Date();
    ...
    for (const contest of contestsForDaily) {
        ...
        const text = (contest.dailyMessage?.trim()) ||
...
export async function sendContestStartNotification(contestId) {
    ...
    const botToken = config.telegramBotToken?.trim();
    if (!botToken)
        return { ok: false, error: "Не задан токен бота (Настройки → Telegram)" };
    const clients = await prisma.client.findMany({
        where: { telegramId: { not: null }, isBlocked: false },
        select: { telegramId: true },
    });
...
export async function sendContestDrawResults(contestId) {
    ...
    if (!contest || contest.winners.length === 0)
        return;
    const config = await getSystemConfig();
    const botToken = config.telegramBotToken?.trim();
    if (!botToken)
...
//# sourceMappingURL=contest-daily-reminder.service.js.map