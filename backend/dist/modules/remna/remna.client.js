...
export function remnaDeleteUser(uuid) {
    return remnaFetch(`/api/users/${uuid}`, { method: "DELETE" });
}
...
export function remnaUsernameFromClient(opts) {
    const sanitize = (s) => s.replace(/[^a-zA-Z0-9_-]/g, "_").slice(0, 36);
...
export function remnaGetUserHwidDevices(userUuid) {
    return remnaFetch(`/api/hwid/devices/${userUuid}`);
}
/** POST /api/hwid/devices/delete — удалить устройство пользователя (Remna HWID) */
export function remnaDeleteUserHwidDevice(userUuid, hwid) {
    return remnaFetch("/api/hwid/devices/delete", {
        method: "POST",
 ...
 */
export function remnaRemoveAllUsersFromInternalSquad(squadUuid) {
...
export function remnaGetUserBandwidthStats(userUuid, start, end) {
    const params = new URLSearchParams({ start, end });
    return remnaFetch(`/api/bandwidth-stats/users/${userUuid}?${params}`);
}
/** GET /api/bandwidth-stats/nodes/{uuid}/users — top users usage on node by range */
export function remnaGetNodeUsersUsage(nodeUuid, start, end, topUsersLimit = 50) {
    const params = new URLSearchParams({
        topUsersLimit: String(topUsersLimit),
        start,
        end,
    });
    return remnaFetch(`/api/bandwidth-stats/nodes/${nodeUuid}/users?${params}`);
}
...
export function remnaGetNodesRealtimeUsage() {
    return remnaFetch("/api/bandwidth-stats/nodes/realtime");
}
/** POST /api/ip-control/fetch-users-ips/{nodeUuid} — start async job to fetch user IPs on a node */
export function remnaFetchUsersIps(nodeUuid) {
    return remnaFetch(`/api/ip-control/fetch-users-ips/${nodeUuid}`, { method: "POST" });
}
/** GET /api/ip-control/fetch-users-ips/result/{jobId} — poll async job result */
export function remnaGetFetchUsersIpsResult(jobId) {
    return remnaFetch(`/api/ip-control/fetch-users-ips/result/${jobId}`);
}
/** GET /api/system/nodes/metrics — Prometheus-style metrics per node */
export function remnaGetNodesMetrics() {
    return remnaFetch("/api/system/nodes/metrics");
}
/** GET /api/users/{uuid} — resolve user by UUID (returns full user with username, etc.) */
export function remnaGetUserByUuid(uuid) {
    return remnaFetch(`/api/users/${uuid}`);
}
