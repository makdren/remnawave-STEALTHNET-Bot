...
function parseMetadataExtraOption(metadata) {
    if (!metadata?.trim())
        return null;
    try {
        const obj = JSON.parse(metadata);
        const extra = obj?.extraOption;
...
function getRemnaLimits(data) {
    if (!data || typeof data !== "object")
        return { trafficLimitBytes: 0, hwidDeviceLimit: null };
 ...
 */
export async function applyExtraOptionByPaymentId(paymentId) {
    if (!isRemnaConfigured())
        return { ok: false, error: "Remna API не настроен", status: 503 };
    ...
    if (!option)
        return { ok: false, error: "Платёж не является покупкой опции", status: 400 };
...
//# sourceMappingURL=extra-options.service.js.map