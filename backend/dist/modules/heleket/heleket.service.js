...
export function isHeleketConfigured(config) {
 ...
 */
export async function createHeleketInvoice(params) {
    ...
    if (urlCallback)
        body.url_callback = urlCallback;
    if (urlSuccess)
        body.url_success = urlSuccess;
    if (urlReturn)
        body.url_return = urlReturn;
    if (additionalData != null && additionalData.length > 0)
        body.additional_data = additionalData.slice(0, 255);
    if (toCurrency)
        body.to_currency = toCurrency.toLowerCase();
    if (network)
 ...
 */
export function verifyHeleketWebhookSignature(apiKey, rawBody, signFromBody) {
    if (!apiKey?.trim() || !signFromBody?.trim())
        return false;
    let parsed;
    try {
    ...
    const receivedSign = parsed.sign;
    if (!receivedSign)
        return false;
    delete parsed.sign;
...
//# sourceMappingURL=heleket.service.js.map