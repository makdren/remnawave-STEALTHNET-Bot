...
import { remnaGetUser, remnaGetInternalSquads, remnaUpdateUser, remnaRevokeUserSubscription, remnaDisableUser, remnaEnableUser, remnaResetUserTraffic, } from "../remna/remna.client.js";
...
function getRemnaUserSquads(data) {
...
function getTelegramId(req) {
    ...
    const b = req.body;
    if (typeof b?.telegramId === "number")
        return b.telegramId;
    return null;
}
async function requireBotAdmin(req, res) {
...
botAdminRouter.patch("/notification-settings", async (req, res) => {
    ...
    const update = {};
    if (data.notifyBalanceTopup !== undefined)
        update.notifyBalanceTopup = data.notifyBalanceTopup;
    if (data.notifyTariffPayment !== undefined)
        update.notifyTariffPayment = data.notifyTariffPayment;
    if (data.notifyNewClient !== undefined)
        update.notifyNewClient = data.notifyNewClient;
    if (data.notifyNewTicket !== undefined)
        update.notifyNewTicket = data.notifyNewTicket;
...
async function downloadTelegramFile(botToken, fileId) {
    ...
    if (!getRes.ok || !getData.ok || !getData.result?.file_path)
        return null;
...
//# sourceMappingURL=bot-admin.routes.js.map