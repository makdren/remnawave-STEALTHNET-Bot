 ...
 */
async function sendTelegramPhoto(botToken, chatId, caption, buffer, mimeType, fileName, replyMarkup) {
 ...
 */
async function sendTelegramDocument(botToken, chatId, caption, buffer, mimeType, fileName, replyMarkup) {
        ...
        if (!botToken) {
            ...
            for (const c of clients) {
                ...
                const send = attachment
                    ? isImage
                        ? await sendTelegramPhoto(botToken, tid, message, attachment.buffer, attachment.mimetype, attachment.originalname, replyMarkup)
                        : await sendTelegramDocument(botToken, tid, message, attachment.buffer, attachment.mimetype, attachment.originalname, replyMarkup)
                ...
                if (send.ok)
                    ...
                    if (result.errors.length < 10)
                        result.errors.push(`Telegram ${tid}: ${send.error ?? "error"}`);
                }
                report();
            }
        }
        ...
        if (!smtpConfig.host || !smtpConfig.fromEmail) {
            ...
            for (const c of clients) {
                ...
                if (send.ok)
                    ...
                    result.failedEmail++;
                    if (result.errors.length < 10)
                        result.errors.push(`Email ${email}: ${send.error ?? "error"}`);
                }
                report();
            }
        }
 ...
 */
export async function getBroadcastRecipientsCount() {
