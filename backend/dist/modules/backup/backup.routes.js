...
import { parseDatabaseUrl, runPgRestore, saveBackupToFile, listBackups, createBackupReadStream, readBackupFile, } from "./backup.service.js";
...
export function registerBackupRoutes(router, asyncRoute) {
    ...
    router.get("/backup/create", asyncRoute(async (_req, res) => {
        ...
        if (!db)
            return res.status(503).json({ message: "Неверный формат DATABASE_URL" });
        try {
    ...
    router.post("/backup/restore", upload.single("file"), asyncRoute(async (req, res) => {
        ...
        if (serverPath && typeof serverPath === "string") {
            ...
            if (!buf || buf.length === 0)
                return res.status(404).json({ message: "Бэкап на сервере не найден" });
            sqlBuffer = buf;
        }
        else {
            const file = req.file;
