...
function backToMenuMarkup(backLabel) {
    return { inline_keyboard: [[{ text: backLabel || "◀️ В меню", callback_data: "menu:main" }]] };
}
export async function sendTelegramToUser(telegramId, text, messageThreadId, replyMarkup) {
...
function getTopicIdForEvent(config, eventType) {
    let raw = null;
    switch (eventType) {
        case "new_client":
            raw = config.notificationTopicNewClients ?? null;
            break;
        case "balance_topup":
        case "tariff_payment":
            raw = config.notificationTopicPayments ?? null;
            break;
        case "new_ticket":
            raw = config.notificationTopicTickets ?? null;
            break;
    }
    if (!raw?.trim())
        return null;
...
async function sendTelegramToAdminsForEvent(eventType, text) {
    const config = await getSystemConfig();
    const groupId = config.notificationTelegramGroupId?.trim();
    if (groupId) {
        const topicId = getTopicIdForEvent(config, eventType);
    ...
    const byId = new Map(prefs.map((p) => [p.telegramId, p]));
    const shouldSend = (telegramId) => {
    ...
    await Promise.all(adminIds
        .filter((id) => shouldSend(id))
        .map((id) => sendTelegramToUser(id, text).catch((e) => {
 ...
 */
export async function notifyBalanceToppedUp(clientId, amount, currency, provider) {
 ...
 */
export async function notifyTariffActivated(clientId, paymentId) {
    const client = await prisma.client.findUnique({
        where: { id: clientId },
        select: { telegramId: true, email: true, telegramUsername: true, id: true, balance: true },
    });
    ...
    if (payment?.tariff?.durationDays)
        lines.push(`📅 Срок: ${payment.tariff.durationDays} дн.`);
    if (payment?.amount != null)
        lines.push(`💵 Сумма: <b>${formatMoney(payment.amount, payment.currency ?? "RUB")}</b>`);
    if (payment?.provider)
        lines.push(`🏦 Провайдер: ${escapeHtml(payment.provider)}`);
    lines.push(`🕐 ${formatDate(new Date())}`);
...
export async function notifyAdminsAboutNewTicket(params) {
    ...
    const preview = params.firstMessage.length > 200
...
export async function notifyAdminsAboutClientTicketMessage(params) {
    ...
    const clientLabel = formatClientLabel(client ?? { id: params.clientId });
    const baseUrl = (config.publicAppUrl || "").replace(/\/+$/, "");
    const preview = params.content.length > 200 ? `${params.content.slice(0, 197)}...` : params.content;
    const lines = [
...
export async function notifyAdminsAboutNewClient(clientId) {
    const client = await prisma.client.findUnique({
        where: { id: clientId },
    ...
    if (client.telegramUsername)
        lines.push(`📱 Username: @${escapeHtml(client.telegramUsername)}`);
    if (client.email)
        lines.push(`📧 Email: ${escapeHtml(client.email)}`);
    if (totalClients != null)
        lines.push(`📊 Всего клиентов: <b>${totalClients}</b>`);
 ...
 */
export async function notifyProxySlotsCreated(clientId, slotIds, tariffName) {
    const client = await prisma.client.findUnique({ where: { id: clientId }, select: { telegramId: true } });
    if (!client?.telegramId || slotIds.length === 0)
        return;
 ...
 */
export async function notifySingboxSlotsCreated(clientId, slotIds, tariffName) {
    const client = await prisma.client.findUnique({ where: { id: clientId }, select: { telegramId: true } });
    if (!client?.telegramId || slotIds.length === 0)
        return;
    ...
    for (let i = 0; i < slots.length; i++) {
        const s = slots[i];
        const link = buildSingboxSlotSubscriptionLink({ publicHost: s.node.publicHost ?? "", port: s.node.port ?? 443, protocol: s.node.protocol ?? "VLESS", tlsEnabled: s.node.tlsEnabled }, { userIdentifier: s.userIdentifier, secret: s.secret }, `${name}-${i + 1}`);
...
export async function notifyAutoRenewSuccess(clientId, tariffName, amount, currency) {
    const client = await prisma.client.findUnique({ where: { id: clientId }, select: { telegramId: true } });
    if (!client?.telegramId)
        return;
...
export async function notifyAutoRenewFailed(clientId, tariffName, reason) {
    const client = await prisma.client.findUnique({ where: { id: clientId }, select: { telegramId: true } });
    if (!client?.telegramId)
        return;
 ...
 */
export async function notifyAutoRenewYookassaSuccess(clientId, tariffName, amount, currency, paymentMethodTitle, balancePortion, cardPortion) {
    const client = await prisma.client.findUnique({ where: { id: clientId }, select: { telegramId: true } });
    if (!client?.telegramId)
        return;
    let text = `🔄 <b>Автопродление успешно (ЮKassa)</b>\n\n` +
 ...
 */
export async function notifyAutoRenewYookassaFailed(clientId, tariffName, error) {
    const client = await prisma.client.findUnique({ where: { id: clientId }, select: { telegramId: true } });
    if (!client?.telegramId)
        return;
    const text = `❌ <b>Автоплатёж ЮKassa не прошёл</b>\n\n` +
 ...
 */
export async function notifyAutoRenewUpcoming(clientId, tariffName, price, currency, daysLeft) {
    const client = await prisma.client.findUnique({
        where: { id: clientId },
    ...
    const text = `⏳ <b>Скоро автопродление</b>\n\n` +
 ...
 */
export async function notifyAutoRenewRetry(clientId, tariffName, price, currency, currentRetry, maxRetries) {
    const client = await prisma.client.findUnique({
        where: { id: clientId },
        select: { telegramId: true, balance: true },
    });
    ...
    const text = `🔄 <b>Не удалось продлить подписку</b>\n\n` +
