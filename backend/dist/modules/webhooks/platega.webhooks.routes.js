...
async function findPlategaPaymentByAnyId(candidateIds) {
...
async function ensureTariffActivation(paymentId) {
    const claim = await prisma.$transaction(async (tx) => {
        ...
        if (!row || row.status !== "PAID" || (!row.tariffId && !row.proxyTariffId && !row.singboxTariffId && !hasExtra)) {
            return { claimed: false, reason: "not_paid_or_no_tariff" };
        }
        const meta = parseMeta(row.metadata);
        if (typeof meta.plategaActivationAppliedAt === "string" && meta.plategaActivationAppliedAt.trim()) {
            return { claimed: false, reason: "already_applied" };
        }
...
plategaWebhooksRouter.post("/platega", async (req, res) => {
    ...
    try {
        ...
        const idObj = (data.data && typeof data.data === "object")
            ? data.data
            : {};
        const statusRaw = pickFirstString(data.status, txObj.status, data.state, data.paymentStatus, data.payment_status, idObj.status, idObj.state);
        const status = (statusRaw ?? "").toUpperCase();
        const transactionId = pickFirstString(data.id, txObj.id, data.transactionId, data.transaction_id, idObj.id, idObj.transactionId, idObj.transaction_id);
        ...
        const candidateIds = [...new Set([payloadId, transactionId, externalId, orderId].filter(Boolean))];
