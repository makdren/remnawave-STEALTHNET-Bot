...
function hasExtraOptionInMetadata(metadata) {
    if (!metadata?.trim())
        return false;
    try {
...
function computeSha1(notificationType, operationId, amount, currency, datetime, sender, codepro, notificationSecret, label) {
    const str = [
        notificationType,
        operationId,
        amount,
...
yoomoneyWebhooksRouter.post("/yoomoney", async (req, res) => {
    ...
    if (!label)
        label = pickFirst(body.order_id) || pickFirst(body.orderId) || pickFirst(body.custom);
    ...
    const expectedHash = computeSha1(notificationType, operationId, amount, currency, datetime, sender, codeproStr, secret, label);
    ...
    let payment = null;
    const paymentSelect = { id: true, clientId: true, amount: true, tariffId: true, proxyTariffId: true, singboxTariffId: true, status: true, metadata: true };
    ...
    if (isTopUp) {
        ...
        if (isExtraOption) {
            ...
            if (result.ok) {
                ...
                console.error("[YooMoney Webhook] Extra option apply failed", { paymentId: payment.id, error: result.error });
            }
        }
            ...
            if (activation.ok) {
                ...
                console.error("[YooMoney Webhook] Tariff activation failed", { paymentId: payment.id, error: activation.error });
            }
        }
    }
...
//# sourceMappingURL=yoomoney.webhooks.routes.js.map